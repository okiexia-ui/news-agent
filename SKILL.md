---
name: frontier-brief-pro
description: 生成 FRONTIER BRIEF 风格的科技/资本/地缘政治日报——抓取新闻 → agent 生成结构化 JSON → 赤陶暖色 A4 HTML/PDF 排版 → 可选邮件发送。触发词：生成日报、frontier brief、今日简报、早报、晚报、日报排版、改进日报版面。也可作为"HTML 模板 + 结构化数据填充 + Playwright 导出 PDF"这套模式的参考实现。
---
...
```
1. scripts/prefetch_news.py     → 抓取新闻，写入 /tmp/frontier_brief_news_cache.txt
2. 【你，agent】                 → 读缓存，按下方 JSON schema 生成结构化简报内容
3. 保存为 /tmp/frontier_brief_data.json
...
需要环境变量 `NEWSAPI_KEY`（见 `.env.example`）。HackerNews / Reddit / arXiv / BBC / Bloomberg / CNBC / Economist RSS 不需要 key。抓取结果写入 `/tmp/frontier_brief_news_cache.txt`。

### Step 2 · 生成结构化 JSON（你来做这步）

读 `/tmp/frontier_brief_news_cache.txt`，基于其中的新闻条目，产出符合下面 schema 的 JSON，写入 `/tmp/frontier_brief_data.json`。
...
python3 scripts/render_and_send.py /tmp/frontier_brief_data.json morning
```
